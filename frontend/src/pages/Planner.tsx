...
const SupplementEditModal: React.FC<SupplementEditModalProps> = ({ item, onSave, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-md">
      <div className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-slate-900 uppercase tracking-widest">Edit Supplement</h2>
          <button onClick={onClose} className="p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center text-slate-400 hover:bg-slate-50 rounded-full active:scale-[0.98] transition-transform duration-150">
            <X className="w-5 h-5" />
          </button>
        </div>
...
const SCHEDULE_TYPES: ScheduleItem["type"][] = ["exercise", "meal", "medication", "rest"];

const ScheduleEditModal: React.FC<ScheduleEditModalProps> = ({ item, onSave, onClose }) => {
  const [draft, setDraft] = useState(item);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-md">
      <div className="w-full max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300">
        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-sm font-bold text-slate-900 uppercase tracking-widest">Edit Schedule Item</h2>
          <button onClick={onClose} className="p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center text-slate-400 hover:bg-slate-50 rounded-full active:scale-[0.98] transition-transform duration-150">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Activity</label>
            <input
              type="text"
...
const Planner: React.FC = () => {
  const { supplements, setSupplements, schedule, setSchedule, toggleSupplement, toggleScheduleItem } = useHealthPlan();
        ...
        <section>
          ...
          <div className="space-y-3">
            {supplements.map((s) => (
              <div key={s.id} className="glass-card rounded-2xl p-4 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-bold ${s.taken ? "line-through text-slate-400" : "text-slate-800"}`}>
                    {s.name}
            ...
            {supplements.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-6">No supplements added yet.</p>
            )}
          </div>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-900 uppercase tracking-widest">Daily Schedule</h2>
            </div>
            <button
          ...
          <div className="space-y-3">
            {sortedSchedule.map((item) => (
              <div key={item.id} className="glass-card rounded-2xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-bold truncate ${item.completed ? "line-through text-slate-400" : "text-slate-800"}`}>
                    {item.activity}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md capitalize ${typeChip(item.type)}`}>
                      {item.type}
                    </span>
                  </div>
                </div>
            ...
            {schedule.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-6">No schedule items added yet.</p>
            )}
          </div>
        </section>
